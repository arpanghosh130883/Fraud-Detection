...
Signupdata_ML.Fraud_Acc_Flag.value_counts()
...
best_preds_DM1 = np.where(preds_DM >= 3.198931517545134e-05, 1, 0)
#0.00380784273147583
#4.1071583837037906e-05
#3.198931517545134e-05  threshold recieved after training 2018-2021 data
...
preds_DM_Marco = Marco_model.predict(dtest)
best_preds_DMold= np.where(preds_DM_Marco >= 0.007600000000000007, 1, 0)
...
#save_model should be used this compatable with Java Wrapper
...
loaded_model_Marco=xgb.Booster()
loaded_model_Marco.load_model("xgboost_Signup_Marco_Java_Encoder_joblib_V1.model")
...
# Comparing with previous model currently present in Production
preds_DM_Marco = loaded_model_Marco.predict(dtest)
best_preds_DMold= np.where(preds_DM_Marco >= 0.007600000000000007, 1, 0)
#0.007600000000000007
...
from sklearn import model_selection, metrics
hardpredtst2020_21_all_columns_DM_Marco=best_preds_DMold
def conf_matrix(y,pred):
    ((tn, fp), (fn, tp)) = metrics.confusion_matrix(y, pred)
    ((tnr,fpr),(fnr,tpr))= metrics.confusion_matrix(y, pred, 
            normalize='true')
    return pd.DataFrame([[f'TN = {tn} (TNR = {tnr:1.2%})', 
            ...
            columns=['Pred 0(Approve as Legit)', 
                            'Pred 1(Deny as Fraud)'])
conf_matrix(y_test,hardpredtst2020_21_all_columns_DM_Marco)
...
from sklearn.metrics import roc_curve, auc
#fpr, tpr, thresholds = roc_curve(train[target].values, y_pred_train)
fpr_test_old, tpr_test_old, thresholds_test_old = roc_curve(y_test, best_preds_DMold)
roc_auc_test_old = auc(fpr_test_old, tpr_test_old) # compute area under the curve
roc_auc_test_old
plot_ROC(fpr_test_old, tpr_test_old,thresholds_test_old,roc_auc_test_old)
...
np.histogram(X_train, breakpoints)[1]
...
percents = df[['Train Percent', 'Prod Percent', 'Bucket']]            .melt(id_vars=['Bucket'])            .rename(columns={'variable':'Population', 'value':'Percent'})
...
np.round(calculate_psi(np_X_train, np_X_test, buckettype='bins', buckets=10, axis=1),5)


# In[512]:


calculate_psi(np_X_train, np_X_test, buckettype='bins', buckets=10, axis=1)
...
from psi_new import calculate_psi_new


# In[526]:


calculate_psi_new(np_X_train, np_X_test, preds_DM, buckettype='bins', buckets=10, axis=1)
